...
(`src/index.ts` → `dist/pf2e-trooper.{js,css}`). `module.json` is the manifest.
...
- Release: tag `vX.Y.Z` → `release.yml` stamps the version, type-checks, builds, publishes `module.json` + `pf2e-trooper.zip` (zip ships `dist lang packs assets` — must include the art).
